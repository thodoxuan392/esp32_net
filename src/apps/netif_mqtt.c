...
// Client Subcribe
static netif_status_t netif_wifi_ethernet_mqtt_subcribe(netif_mqtt_client_t* client, char* topic,
														uint8_t qos);
// Client Unsubcribe
static netif_status_t netif_wifi_ethernet_mqtt_unsubcribe(netif_mqtt_client_t* client, char* topic);
// Client Publish
static netif_status_t netif_wifi_ethernet_mqtt_publish(netif_mqtt_client_t* client, char* topic,
													   char* payload, uint8_t qos, uint8_t retain);
	#endif
	#if defined(NETIF_4G_ENABLE) && NETIF_4G_ENABLE == 1
...
// Client Publish
static netif_status_t netif_4g_mqtt_publish(netif_mqtt_client_t* client, char* topic, char* payload,
 ...
 * @return netif_status_t Status of Process
 */
netif_status_t netif_mqtt_publish(netif_mqtt_client_t* client, char* topic, char* payload,
								  uint8_t qos, uint8_t retain)
{
	netif_manager_mode_t netmanager_mode = netif_manager_get_mode();
	switch(netmanager_mode)
	...
	switch(state)
	{
		case STATE_WIFI_ETHERNET_MQTT_CONFIG:
			if(NETIF_GET_TIME_MS() - last_time_sent > NETIF_APPS_RETRY_INTERVAL)
			{
				...
				size = sprintf(at_message, NETIF_ATCMD_WIFI_ETHERNET_MQTT_CLIENT_CONFIG, 1,
		...
		case STATE_WIFI_ETHERNET_WAIT_FOR_RESPONSE:
			...
			if(netif_core_atcmd_is_responded(NETIF_WIFI_ETHERNET, &response))
			{
				...
				{ // Force Error is Sucess because don't have read command to get client status
					netif_core_atcmd_reset(NETIF_WIFI_ETHERNET, true);
					retry = 0;
					state = STATE_WIFI_ETHERNET_MQTT_CONFIG;
					return NETIF_OK;
	...
	switch(state)
	{
		case STATE_WIFI_ETHERNET_MQTT_CONNECT:
			if(NETIF_GET_TIME_MS() - last_time_sent > NETIF_APPS_RETRY_INTERVAL)
			{
				...
				size = sprintf(at_message, NETIF_ATCMD_WIFI_ETHERNET_MQTT_CONNECT, client->host,
							   client->port, client->reconnect);
				netif_core_wifi_ethernet_output(at_message, size);
				state = STATE_WIFI_ETHERNET_WAIT_FOR_RESPONSE;
			}
			break;
	...
	switch(state)
	{
		case STATE_WIFI_ETHERNET_MQTT_SUBCRIBE:
			if(NETIF_GET_TIME_MS() - last_time_sent > NETIF_APPS_RETRY_INTERVAL)
			{
				...
				netif_core_atcmd_reset(NETIF_WIFI_ETHERNET, true);
				// Send Subcribe Topic
				size = sprintf(at_message, NETIF_ATCMD_WIFI_ETHERNET_MQTT_SUBCRIBE, topic, qos);
				netif_core_wifi_ethernet_output(at_message, size);
				state = STATE_WIFI_ETHERNET_WAIT_FOR_RESPONSE;
			}
			break;
	...
	switch(state)
	{
		case STATE_WIFI_ETHERNET_MQTT_UNSUBCRIBE:
			if(NETIF_GET_TIME_MS() - last_time_sent > NETIF_APPS_RETRY_INTERVAL)
			{
				...
				size = sprintf(at_message, NETIF_ATCMD_WIFI_ETHERNET_MQTT_UNSUBCRIBE, topic);
				netif_core_wifi_ethernet_output(at_message, size);
				state = STATE_WIFI_ETHERNET_WAIT_FOR_RESPONSE;
			}
			break;
	...
	switch(state)
	{
		case STATE_WIFI_ETHERNET_MQTT_PUBLISH:
			if(NETIF_GET_TIME_MS() - last_time_sent > NETIF_APPS_RETRY_INTERVAL)
			{
				...
				size = sprintf(at_message, NETIF_ATCMD_WIFI_ETHERNET_MQTT_PUBLISH, topic,
	...
	switch(state)
	{
		...
		case STATE_4G_MQTT_CONFIG:
			if(NETIF_GET_TIME_MS() - last_time_sent > NETIF_APPS_RETRY_INTERVAL)
			{
				...
				netif_core_atcmd_reset(NETIF_4G, true);
				// Send Client Config to AP to 4G Module
				size = sprintf(at_message, NETIF_ATCMD_4G_MQTT_ACCQ, client->client_id);
				utils_log_debug(at_message);
				netif_core_4g_output(at_message, size);
				state = STATE_4G_MQTT_WAIT_FOR_RESPONSE;
			}
	...
	switch(state)
	{
		case STATE_4G_MQTT_CONNECT:
			if(NETIF_GET_TIME_MS() - last_time_sent > NETIF_APPS_RETRY_INTERVAL)
			{
				...
				size = sprintf(at_message, NETIF_ATCMD_4G_MQTT_CONNECT, client->host, client->port,
							   client->keep_alive, client->clean_session, client->username,
							   client->password);
				utils_log_debug(at_message);
				netif_core_4g_output(at_message, size);
				state = STATE_4G_MQTT_WAIT_FOR_RESPONSE;
	...
	switch(state)
	{
		case STATE_4G_MQTT_SUBCRIBE_TOPIC:
			if(NETIF_GET_TIME_MS() - last_time_sent > NETIF_APPS_RETRY_INTERVAL)
			{
				...
				size = sprintf(at_message, NETIF_ATCMD_4G_MQTT_SUBSCRIBE, strlen(topic), qos);
				netif_core_4g_output(at_message, size);
	...
	switch(state)
	{
		case STATE_4G_MQTT_UNSUBCRIBE_TOPIC:
			if(NETIF_GET_TIME_MS() - last_time_sent > NETIF_APPS_RETRY_INTERVAL)
			{
				...
				size = sprintf(at_message, NETIF_ATCMD_4G_MQTT_UNSUBSCRIBE_TOPIC, strlen(topic));
				netif_core_4g_output(at_message, size);
	...
	switch(state)
	{
		case STATE_4G_MQTT_PUBLISH_TOPIC:
			if(NETIF_GET_TIME_MS() - last_time_sent > NETIF_APPS_RETRY_INTERVAL)
			{
				...
				size = sprintf(at_message, NETIF_ATCMD_4G_MQTT_PUBLISH_TOPIC, strlen(topic));
				netif_core_4g_output(at_message, size);
		...
		case STATE_4G_MQTT_PUBLISH_TOPIC_INPUT:
			if(NETIF_GET_TIME_MS() - last_time_sent > NETIF_ATCMD_TIMEOUT)
			{
				...
				{
					utils_log_error(
						"MQTT publish topic input response timeout, maximum retried time\r\n");
					// Reset Buffer and Indication if Try number over
					retry = 0;
		...
		case STATE_4G_MQTT_PUBLISH_PAYLOAD:
			...
			if(netif_core_atcmd_is_responded(NETIF_4G, &response))
			{
				if(response == NETIF_RESPONSE_OK)
				{
					...
					size =
						sprintf(at_message, NETIF_ATCMD_4G_MQTT_PUBLISH_PAYLOAD, strlen(payload));
					netif_core_4g_output(at_message, size);
					retry = 0;
		...
		case STATE_4G_MQTT_PUBLISH_PAYLOAD_INPUT:
			...
			if(netif_core_atcmd_is_responded(NETIF_4G, &response))
			{
				...
				else if(response == NETIF_RESPONSE_ERROR)
				{
					...
					{
						utils_log_error(
							"MQTT publish payload input got error, maximum retried time\r\n");
						retry = 0;
						state = STATE_4G_MQTT_PUBLISH_TOPIC;
						return NETIF_FAIL;
					}
		...
		case STATE_4G_MQTT_PUBLISH:
			...
			if(netif_core_atcmd_is_responded(NETIF_4G, &response))
			{
				if(response == NETIF_RESPONSE_OK)
				{
					...
					last_time_sent = NETIF_GET_TIME_MS();
					// Publish Message
					size = sprintf(at_message, NETIF_ATCMD_4G_MQTT_PUBLISH, qos,
								   120, // timeout
								   retain, // retain
								   0); // dup
					netif_core_4g_output(at_message, size);
		...
		case STATE_4G_MQTT_WAIT_FOR_RESPONSE:
			...
			if(netif_core_atcmd_is_responded(NETIF_4G, &response))
			{
				...
				else if(response == NETIF_RESPONSE_ERROR)
				{
					...
					{
						utils_log_error(
							"MQTT wait for response got error, maximum retried time\r\n");
						retry = 0;
						state = STATE_4G_MQTT_PUBLISH_TOPIC;
						return NETIF_FAIL;
					}
	...
	switch(state)
	{
		...
		case 0:
			if(netif_core_atcmd_get_data_after(NETIF_4G,
											   &on_message_buffer[on_message_buffer_index++]))
			{
				// Check Pattern is match or not
				if(utils_string_is_receive_data(on_message_buffer, on_message_buffer_index,
												topic_pattern))
				{
